...
def main():
    ...
    print(args)
    #     cc_feature1,cc2 = CC_feature(args.dataset_name)
    #     cc=np.concatenate((cc_feature1,cc2),axis=1)
    #     cc_feature=torch.tensor(cc)
    ...
    for run in range(args.runs):
        ...
        f = spatial_embeddings(data)

        if args.dataset_name == 'pubmed':
            f1 = ContextualPubmed(data)
            concatenated_list = np.concatenate((f1, f), axis=1)
        else:
