...
def spatial_embeddings(data,test_index,val_index):
    ...
    for idx_val in val_index:
        label[idx_val] = test_class
...
def Contextual_embeddings(data, dataset_name,train_idx):
...
def Contextual_embeddings_eucledian(data,train_idx):
    Domain_Fec = pd.DataFrame(data.x.numpy())
    label = pd.DataFrame(data.y.numpy(), columns=['class'])
    Data = pd.concat([Domain_Fec, label], axis=1)
    Number_nodes = len(data.y)
